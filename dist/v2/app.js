...
function card(c){
 ...
 return `<article class="company-listing">
  ...
  <div class="listing-content">
   ...
   <div class="listing-footer"><a class="listing-offers" href="${href}#offers">${c.offer.length} შეთავაზება ${icon('chevron-right')}</a><a class="button listing-action" href="${href}" aria-label="${esc(c.name)} — გაცნობა">კომპანიის ნახვა ${icon('arrow-up-right')}</a></div>
  </div>
 </article>`;
}
...
function renderResults(){const grid=document.querySelector('#results-grid');if(!grid)return;
 for(const key of ['industry','type','city','collaboration','format','language']){
  const any={industry:'ყველა კატეგორია',type:'ყველა ტიპი',city:'ყველა ქალაქი',collaboration:'ნებისმიერი',format:'ნებისმიერი',language:'ყველა ენა'}[key];
 ...
 const industry=filterOptions.industry[filters.industry];document.querySelector('#catalog-title').textContent=industry||'კომპანიები';document.title=(industry||'კატეგორიები და კომპანიები')+' — MeetAny';const results=getResults(filters);document.querySelector('#result-count').setAttribute('role','status');document.querySelector('#result-count').setAttribute('aria-live','polite');document.querySelector('#result-count').innerHTML=`<strong>${results.length} კომპანია</strong> <span>· ${results.reduce((sum,c)=>sum+c.offer.length,0)} შეთავაზება</span>`;
...
if(document.querySelector('#results-grid')){
 ...
 document.querySelector('#catalog-search').addEventListener('submit',e=>{e.preventDefault();setFilters({q:queryInput.value.trim()});queryInput.blur();document.querySelector('.results-toolbar').scrollIntoView({block:'start'});});
...
function openRequest(companyId,offerIndex){
 ...
 <form id="intro-form" novalidate>
  <div class="request-essential">
   ...
   <div class="request-field-grid">
    <div class="field"><label for="request-name">შენი ან კომპანიის სახელი *</label><input id="request-name" name="name" required maxlength="120" value="${esc(d.name||'')}" autocomplete="organization" placeholder="შენი სახელი ან კომპანია"></div>
  ...
  <div class="form-action-bar"><button type="button" class="form-cancel" data-close>გაუქმება</button><button class="button form-submit" type="submit">გადაამოწმე დეტალები ${icon('arrow-right')}</button></div>
  <p class="request-demo-note">მოთხოვნა არ იგზავნება — შეგიძლია ჩამოტვირთო.</p>
 </form>`;
 ...
 form.addEventListener('submit',e=>{
  ...
  document.querySelector('#form-content').innerHTML=`<span class="section-kicker">მოთხოვნის მონახაზი</span><h2 id="form-title" tabindex="-1">გადაამოწმე დეტალები</h2>
   ...
   <p class="form-note">მოთხოვნა არ გაგზავნილა. ჩამოტვირთე ან შეასწორე დეტალები.</p>
   <div class="form-actions"><button class="button" data-action="download">ჩამოტვირთე მოთხოვნა ${icon('download')}</button><button class="button button-outline" id="edit-request">რედაქტირება</button></div>`;
...
function openCompanyForm(draft={}){document.querySelector('#form-content').innerHTML=`<span class="section-kicker">შენი კომპანია MeetAny-ზე</span><h2 id="form-title">კომპანიის დამატება</h2><p class="request-intro">შეავსე სამი ველი და გადაამოწმე პროფილი.</p><form id="company-form"><div class="company-essential"><div class="field"><label for="company-name">კომპანიის დასახელება *</label><input id="company-name" name="name" required maxlength="80" value="${esc(draft.name||'')}" autocomplete="organization" placeholder="მაგ.: შენი კომპანიის სახელი"></div><div class="field"><label for="company-offer">რა პროდუქტს ან მომსახურებას სთავაზობ? *</label><textarea id="company-offer" name="offer" required minlength="10" maxlength="1500" rows="3" placeholder="მოკლედ აღწერე პროდუქტი ან მომსახურება">${esc(draft.offer||'')}</textarea></div><div class="field"><label for="company-area">სად მუშაობს შენი კომპანია? *</label><input id="company-area" name="area" value="${esc(draft.area||'')}" required maxlength="100" placeholder="მაგალითად: მთელი საქართველო"></div></div><div class="form-action-bar"><button type="button" class="form-cancel" data-close>გაუქმება</button><button class="button form-submit" type="submit">გადაამოწმე დეტალები ${icon('arrow-right')}</button></div><p class="request-demo-note">პროფილის მონახაზი — საჯაროდ არ გამოქვეყნდება.</p></form>`;document.querySelector('#company-form').addEventListener('submit',e=>{e.preventDefault();const data=readValidDraft(e.target);if(!data)return;lastDraft={profile:true,...data};document.querySelector('#form-content').innerHTML=`<span class="section-kicker">პროფილის მონახაზი</span><h2 id="form-title">${esc(data.name)}</h2><p style="white-space:pre-wrap">${esc(data.offer)}</p><div class="detail-meta"><div><small>სამუშაო არეალი</small>${esc(data.area)}</div></div><p class="form-note">პროფილი არ გამოქვეყნებულა. ეს ტექსტის წინასწარი ვერსიაა.</p><div class="form-actions"><button class="button" data-action="download">ჩამოტვირთე პროფილი ${icon('download')}</button><button class="button button-outline" id="edit-company">რედაქტირება</button></div>`;document.querySelector('#edit-company').addEventListener('click',()=>openCompanyForm(data));document.querySelector('#form-dialog').scrollTop=0;});const dialog=document.querySelector('#form-dialog');if(!dialog.open)dialog.showModal();dialog.scrollTop=0;}
...
if(menuButton){
 ...
 drawer.addEventListener('close',()=>{document.documentElement.classList.remove('drawer-open');menuButton.setAttribute('aria-expanded','false');menuButton.focus({preventScroll:true});});
 window.matchMedia('(min-width:721px)').addEventListener('change',e=>{if(e.matches&&drawer.open)close();});
}
